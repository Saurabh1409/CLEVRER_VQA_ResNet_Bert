...
if __name__=='__main__':
    ...
    word2idx = read_json("Dataloader/word2idx.json")
    test_json_data = read_json(TEST_JSON_PATH)
    test_data_desc= json_preprocess(test_json_data,True)
    test_src = Clevrer_Dataset(test_data_desc,TEST_FRAME_PATH,word2idx,True)
    ...
    resnet = Resnet(768)
    lstm = Lstm_vid(768,768)
    bert = Bert()
    vocab_dim = len(word2idx)
    model = Model_Desc(resnet,lstm,bert,vocab_dim,768)

    model.load_state_dict(torch.load("resnet_bert_desc_best_1105_lr01.pth"))
    ...
    with torch.no_grad():
        ...
        for batch in tqdm(test_dataloader):
            ...
            out_des = model(vid,quest,quest_type)
            predict.extend(out_des.tolist())
        df_desc = pd.DataFrame(list(zip(scene_idx,question_id,predict)),columns = ['Scene','Q_id','Pred'])
        df_desc.to_json("Des.json")